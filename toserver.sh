...
if [ "$site" == "p40" ]; then
  rsync -avh -e "ssh -p 22 -o ProxyCommand='ssh -p 8695 zhangwenbo5@120.92.114.84 -W %h:%p'" --exclude-from='.gitignore' ./nn_se/* zhangwenbo5@ksai-P40-2:/home/zhangwenbo5/lihongfeng/ulaw-SGAN-for-SE/$2
elif [ "$site" == "v100-3" ]; then
  rsync -avh -e "ssh -p 22 -o ProxyCommand='ssh -p 8695 zhangwenbo5@120.92.114.84 -W %h:%p'" --exclude-from='.gitignore' ./nn_se/* zhangwenbo5@ksai-v100-3:/home/zhangwenbo5/lihongfeng/ulaw-SGAN-for-SE/$2
elif [ "$site" == "15123" ] || [ "$site" == "15041" ] || [ "$site" == "15043" ]; then
  echo "To $user@$site:~/worklhf/ulaw-SGAN-for-SE/$2"
  rsync -avh -e 'ssh -p '$site --exclude-from='.gitignore' ./nn_se/* $user@speaker.is99kdf.xyz:~/worklhf/ulaw-SGAN-for-SE/$2
elif [ "$site" == "15043ali" ]; then
  echo "To $user@$site:~/worklhf/ulaw-SGAN-for-SE/$2"
  rsync -avh -e 'ssh -p 6662' --exclude-from='.gitignore' ./nn_se/* $user@47.92.169.196:~/worklhf/ulaw-SGAN-for-SE/$2
