...
def eval_testSet_by_meta(mix_SNR, save_test_records=False):
  ...
  if not save_test_records:
    enhanced_testsets_save_dir = None
  else:
    _dir = misc_utils.enhanced_testsets_save_dir()
    enhanced_testsets_save_dir = str(_dir)
    if _dir.exists():
      import shutil
      shutil.rmtree(str(_dir))
    _dir.mkdir()
  eval_testSet_by_list(meta_list, mix_SNR, enhanced_testsets_save_dir)
